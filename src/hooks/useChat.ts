...
import { chatUserApi } from "api/chat";
...
export default function useChat() {
  const { id: userId, receiverId } = useParams();
  ...
  const io = useMemo(() => socket(userId!), []); // eslint-disable-line
  ...
  const fetchMessages = async () => {
    ...
    const { data: response, error } = await chatUserApi(
      userId!,
      receiverId!,
      data.limit,
