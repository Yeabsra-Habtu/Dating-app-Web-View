...
export default function Chat() {
  ...
  return (
    <div className="w-full min-h-screen">
      <header className="fixed bg-white z-10 top-0 left-0 w-full px-4 py-3 shadow-md">
        <div className="flex w-full items-center gap-x-3">
          ...
          <div className="w-full">
            <p className="text-base font-semibold text-gray-00">
              {data.data[0]?.anonymiseUserId === data.receiver._id
                ? "Anonymous"
                : data.receiver.name}
            </p>
      ...
      <footer className="fixed bg-white bottom-0 left-0 w-full border-t border-t-gray-100 px-2 py-3 flex items-end gap-x-3 shadow-2xl">
        <Input
          value={input}
          onChange={handleInputChange}
          type="text"
          className="w-full bg-gray-200 rounded-md px-4 py-3 text-sm font-normal border border-gray-400 outline-none"
          placeholder="Type a message..."
        />
        <Button
          disabled={input.trim().length === 0}
          onClick={handleSendMessage}
          className="h-full py-2.5 px-4 text-2xl flex items-center justify-center"
          type="primary"
        >
          <SendIcon />
        </Button>
...
function SenderMessage({ chat }: { chat: IChat }) {
  return (
    <div className="w-full flex items-start gap-x-2.5 mt-4 justify-end">
      <div className="w-full max-w-80 p-4 bg-blue-600 text-white rounded-s-xl rounded-ee-xl">
...
function ReceiverMessage({ chat }: { chat: IChat }) {
  return (
    <div className="w-full flex items-start gap-x-2.5 mt-4">
      ...
      />
      <div className="w-full max-w-80 p-4 bg-gray-200 rounded-e-xl rounded-es-xl">
        <div className="w-full flex gap-x-2 items-center">
